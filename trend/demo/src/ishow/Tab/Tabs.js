...
export default class Tabs extends Component {
  ...
  render(){
    ...
    const { type, addable, closable, editable,isAllInOne } = this.props;
    ...
    return (
      <div style={this.style()} className={this.className(tabsCls)}>
        <div className={isAllInOne?"ishow-tabs__header isAllInOneTabHeader":"ishow-tabs__header"}>
          {addButton}
              ...
              <div className="ishow-tabs__nav" ref="nav" style={navStyle}>
                {
                  React.Children.map(children, (item, index) => {
                    const { name, label, disabled, customClass } = item.props;
